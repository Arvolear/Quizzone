...
function getRepublishCell($category)
{
    return "<form autocomplete=\"off\" action=\"" . htmlspecialchars($_SERVER['PHP_SELF']) . "\" method=\"post\">                    
                    <input type=\"hidden\" name=\"republish_category\" value=" . "\"" . $category . "\">
                    <input class=\"green_submit\" type=\"submit\" value=\"View\" name=\"republish\">
                </form>";
}
    ...
    if (mysqli_num_rows($result) > 0) {
        ...
        while ($row = mysqli_fetch_array($result)) {
            ...
            if ($what == "production") {
                ...
                $DB_OUTPUT .= "<td class=\"tableButton\">" . getRepublishCell($row['category']) . "</td>";
                $DB_OUTPUT .= "<td class=\"tableButton\">" . getQuestionsCell($row['category']) . "</td>";
            }
...
function getAll()
{
    ...
    getCategories($FINISHED_CATEGORIES, "Finished quizzes", "finished");
}
...
function publish($category)
{
    ...
    header("Location: timed_publish.php", true, 303);
    exit();
}

function republish($category)
{
    $_SESSION['publish_category'] = $category;

    header("Location: timed_republish.php", true, 303);
    exit();
}
...
function unpublish($category)
{
    ...
    header("Location: timed_unpublish.php", true, 303);
    exit();
}
...
if (
    ...
    if (isset($_POST['add'])) {
        ...
        republish($_POST['republish_category']);
